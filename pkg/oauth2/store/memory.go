...
type inMemoryDeviceCodeStore struct {
	mu          sync.RWMutex
	codes       map[string]oauth2.DeviceCode
...
func NewInMemoryDeviceCodeStore() oauth2.DeviceCodeStore {
	store := &inMemoryDeviceCodeStore{
		codes:       make(map[string]oauth2.DeviceCode),
...
func (s *inMemoryDeviceCodeStore) Save(code oauth2.DeviceCode) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *inMemoryDeviceCodeStore) Get(deviceCode string) (oauth2.DeviceCode, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *inMemoryDeviceCodeStore) GetByUserCode(userCode string) (oauth2.DeviceCode, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *inMemoryDeviceCodeStore) Update(code oauth2.DeviceCode) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.codes[code.DeviceCode]; !ok {
		return oauth2.ErrDeviceCodeNotFound
	}
	s.codes[code.DeviceCode] = code
	return nil
}
	...
	for {
		...
		case <-ticker.C:
			...
			for deviceCode, code := range s.codes {
				if now.Sub(code.CreatedAt) > oauth2.DeviceCodeExpiry {
					delete(s.codes, deviceCode)
					delete(s.userCodes, code.UserCode)
				}
			}
