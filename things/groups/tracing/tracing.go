...
// CreateGroups traces the "CreateGroups" operation of the wrapped policies.Service.
func (tm *tracingMiddleware) CreateGroups(ctx context.Context, token string, g ...mfgroups.Group) ([]mfgroups.Group, error) {
	ctx, span := tm.tracer.Start(ctx, "svc_create_group")
	defer span.End()

	return tm.gsvc.CreateGroups(ctx, token, g...)
}

// ViewGroup traces the "ViewGroup" operation of the wrapped policies.Service.
...
// UpdateGroup traces the "UpdateGroup" operation of the wrapped policies.Service.
func (tm *tracingMiddleware) UpdateGroup(ctx context.Context, token string, g mfgroups.Group) (mfgroups.Group, error) {
	ctx, span := tm.tracer.Start(ctx, "svc_update_group", trace.WithAttributes(attribute.String("id", g.ID)))
	defer span.End()

	return tm.gsvc.UpdateGroup(ctx, token, g)
}

// EnableGroup traces the "EnableGroup" operation of the wrapped policies.Service.
...
// DisableGroup traces the "DisableGroup" operation of the wrapped policies.Service.
