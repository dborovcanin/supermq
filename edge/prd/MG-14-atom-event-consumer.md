...
Earlier drafts of both PRDs treated this as unavoidable, on the basis that no
entity lifecycle events exist: all Magistrala event consumers were deleted in
...
| `entity.create`, `entity.delete` | attachment cache |
...
- A cache-invalidation interface both MG-07 and MG-08 can register against.
...
1. Atom publishes events in the Magistrala compose stack; the exchange receives
   them.
2. Re-homing a device from gateway A to B invalidates the attachment cache, and A
   is denied on its next publish without waiting for the TTL.
3. Approving a pending device invalidates the cache, and it can publish
   immediately.
...
7. With the broker stopped, both caches still expire by TTL and the system stays
...
- **Outbox lag.** `ATOM_EVENTS_OUTBOX_POLL_INTERVAL_SECS` defaults to 5s
  (`atom/docker-compose.yml:57`), so invalidation is not instantaneous. Fine for
  both caches, but it means "immediately" in the acceptance criteria means
