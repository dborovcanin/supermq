...
// If the request fails, it logs the error.
func (lm *loggingMiddleware) ListMemberships(ctx context.Context, token, groupID string) (mp mfgroups.Memberships, err error) {
	defer func(begin time.Time) {
	...
	return lm.svc.ListMemberships(ctx, token, groupID)
}
