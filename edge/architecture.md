...
> **One format constraint, and only one.** The serial travels verbatim in the
> topic, so it **must not contain `/`** — that is the topic separator. Everything
> else passes through untouched: uppercase, `.`, `-`, `:`, unicode.
>
> Atom itself constrains nothing (A6); the restriction is Magistrala's and is
> enforced at device creation. A real serial containing `/` must be given a
> different `external_id` — percent-encoding was rejected because it would put
> encode/decode on the publish path and break "verbatim".
...
| Which devices are declared on this gateway? | `entities(attributesContains: { gateways: ["018a-gw-a"] })` — JSONB array containment, **needs A1** |
...
hold `unit 7 → WM-2024-ABC.123`, and per A3 that lives in agent-side config —
Magistrala never stores it.

Generating that config needs an authoritative list of *which serials belong to
this gateway*, which is exactly the declared relation:
...
### 3.6 Fault correlation — a named future view
...
**Devices** — declared and observed, merged, each row carrying the status from
§2.3: healthy, silent, or undeclared. Drill down to a device's messages, or
filter to what arrived *via this gateway*.
...
| **A2** | Scoping filters on `authorizedObjectIds` | The query struct and repository support `attributesContains`, `profileId`, `parentGroupId`, `includeDescendants`; the resolver pins them to `None` (`graphql/authz.rs:47-61`) | Plumbing |
...
Local addressing — BLE MAC, Modbus unit — stays entirely on the gateway. The
cloud addresses devices by serial; the gateway resolves serial → local address
itself, so no protocol-specific knowledge enters Magistrala.
...
| Layer | Change |
|---|---|
| **Atom** | A1 `attributesContains` on entities — **required**, backs the gateway→devices lookup |
| **Atom** | A2 scoping filters on `authorizedObjectIds` |
| **Atom** | A3 `directPolicies(objectId:)` reverse lookup |
| **Atom** | A4 many-to-many object group membership |
| **Atom** | A6 `external_id`, unique per tenant |
| **`pkg/atom`** | Device Type API — create, version, bind, list |
| **`pkg/atom`** | Group membership, hierarchy, object/principal groups |
| **`pkg/atom`** | Group-scoped permission blocks |
| **`pkg/atom`** | Fix `objectType` → `entity:device`; policy pagination; entity applicability |
| **`pkg/atom`** | Consume Atom domain events for cache invalidation |
| **Messaging** | `d` topic segment; `device_id` on `Message`, carried verbatim — no lookup |
| **Storage** | `device_id` column + index; `DeviceIDs` reader filter |
| **Readers** | Intersect device filter with the authorized set — **security fix** |
| **Readers** | Gateway device view — declared ∪ observed, merged with status |
| **API/SDK/CLI** | Device, Gateway, DeviceType surfaces; the device→gateways relation; retire Client |
| **Bootstrap** | One attribute: `gateway_id` on the config projection (§5.2) |

### The security fix is not optional

Reader authorization is **not a boundary today**.
`readers/api/http/transport.go:250-266` authorizes `subscribe` on the *channel*,
then applies the caller-supplied `publisher` / `publishers` filters without
validating them (`readers/messages.go:48-49`). Any user who can read a channel
...
| # | Question |
|---|---|
| **A4** | Global vs tenant-scoped device types — shapes the API, hard to widen after 1.0 |
| **B2** | Reader cache TTL — this *is* the revocation SLA, a customer-visible number |
| **C5** | Accept reserving `d` as a first subtopic segment |
| **C6** | Does the `Connection` proto need a device field |
| **E1** | Telemetry validation against the device type — documentation, at minimum |
| **E2** | Is the capability model expressive enough? Model two or three more real device types before freezing |
| **E4** | Downlink command path |

Raised by the `external_id` decision and needed before it merges — all one-way
doors:

- **Case sensitivity.** `ABC123` and `abc123`: one device or two? Choosing
  case-insensitive merges devices that no migration can separate.
- **Whitespace normalisation.** `"ABC123 "` and `"ABC123"` are two devices unless
  trimmed.
- **Mutability.** The serial is denormalised onto every message row, so changing
  it orphans that device's history. Forbid the change, or accept the break?

Raised by the device→gateway relation:

- **Stale references.** Deleting a gateway leaves its ID in every device that
  named it. Resolve-and-drop on read is enough for correctness; a sweep on
  gateway deletion keeps the data honest. Decide which, and whether stale entries
...
| A3 | Model local transport addressing? | 🟡 | MG-12 | ✅ **No — agent-side only** |
...
#### A3 🟡 Does Magistrala model local transport addressing?
